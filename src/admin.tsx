    ...
    <motion.div initial={{opacity:0}} animate={{opacity:1}} className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm grid place-items-center p-4" onClick={onClose}>
      <motion.div initial={{scale:.9}} animate={{scale:1}} className="glass w-full max-w-2xl p-6 max-h-[90vh] overflow-auto" onClick={e=>e.stopPropagation()}>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <input className="input sm:col-span-2" placeholder="Image URL (Unsplash recommended)" value={f.image} onChange={e=>setF({...f,image:e.target.value})}/>
          ...
          <div className="sm:col-span-2 border-t border-white/10 pt-3">
            <div className="font-display text-sm mb-2">Attachments (certificates, PDFs, docs)</div>
            ...
            <div className="grid grid-cols-12 gap-2 mt-2">
              <input className="input col-span-4" placeholder="File name" value={newAtt.name} onChange={e=>setNewAtt({...newAtt,name:e.target.value})}/>
              <input className="input col-span-5" placeholder="URL / Link" value={newAtt.url} onChange={e=>setNewAtt({...newAtt,url:e.target.value})}/>
              <select className="input col-span-2 !py-1 !px-1 text-xs" value={newAtt.type} onChange={e=>setNewAtt({...newAtt,type:e.target.value})}>
              ...
              <button type="button" onClick={addAtt} className="col-span-1 btn-neon !py-1 !px-2 !text-xs" data-cursor="hover"><Plus className="w-3 h-3"/></button>
            </div>
          </div>
        </div>
        <div className="flex justify-end gap-2 mt-4">
          <button onClick={onClose} className="btn-ghost" data-cursor="hover">Cancel</button>
          <button onClick={()=>onSave({ ...f, tags: f.tags ? f.tags.split(",").map((t:string)=>t.trim()).filter(Boolean) : [], image: f.image || undefined })} className="btn-neon" data-cursor="hover"><Check className="w-4 h-4"/>Save</button>
        </div>
      </motion.div>
    </motion.div>
  );

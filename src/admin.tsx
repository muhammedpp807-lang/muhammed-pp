...
import {
  ...
  GraduationCap, Code, Target, Upload, Loader2
} from "lucide-react";
import {
...
function ItemForm({ item, type, onClose, onSave }:{item:Item|null; type:Item["type"]; onClose:()=>void; onSave:(d:any)=>void}) {
  ...
  const [uploading, setUploading] = useState<string | null>(null); // "image" | "video" | "attach"
  ...
  // File upload — uses Firebase Storage (falls back to data URL)
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, kind: "image" | "video" | "attachment") => {
    const file = e.target.files?.[0]; if (!file) return;
    setUploading(kind); setUploadPct(0);
    try {
      // Try Firebase Storage
      const folder = kind === "image" ? "images" : kind === "video" ? "videos" : "attachments";
      const url = await upload(folder, file, (pct) => setUploadPct(pct));
      if (kind === "image") setF({ ...f, image: url });
      else if (kind === "video") setF({ ...f, video: url });
      else setF({ ...f, attachments: [...f.attachments, { name: file.name, url, type: file.name.split(".").pop() || "file" }] });
    } catch {
      // Fallback to data URL if Firebase fails
      const reader = new FileReader();
      reader.onload = () => {
        const dataUrl = reader.result as string;
        if (kind === "image") setF({ ...f, image: dataUrl });
        else if (kind === "video") setF({ ...f, video: dataUrl });
        else setF({ ...f, attachments: [...f.attachments, { name: file.name, url: dataUrl, type: file.name.split(".").pop() || "file" }] });
      };
      reader.readAsDataURL(file);
    }
    setUploading(null); setUploadPct(0);
  };

  const handleMultipleImages = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files || []);
    for (const file of files) {
      setUploading("image"); setUploadPct(0);
      try {
        const url = await upload("images", file, (pct) => setUploadPct(pct));
        setF((prev:any) => ({ ...prev, images: [...(prev.images||[]), url] }));
      } catch {
        const reader = new FileReader();
        reader.onload = () => setF((prev:any) => ({ ...prev, images: [...(prev.images||[]), reader.result as string] }));
        reader.readAsDataURL(file);
      }
    }
    setUploading(null); setUploadPct(0);
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <div className="sm:col-span-2 border border-white/10 rounded-xl p-3 space-y-2">
            ...
            {f.image ? (
              ...
              <div className="flex flex-wrap gap-3 items-center">
                <label className={`btn-ghost cursor-pointer !py-2 !px-4 ${uploading==="image"?"opacity-60 pointer-events-none":""}`}>
                  {uploading==="image"?<Loader2 className="w-4 h-4 animate-spin"/>:<Upload className="w-4 h-4"/>} {uploading==="image"?`Uploading ${uploadPct}%`:"Upload image"}
                  <input type="file" accept="image/*" className="hidden" onChange={e=>handleFileUpload(e,"image")} disabled={uploading!=null}/>
                </label>
          ...
          {(type === "project" || type === "event" || type === "gallery") && (
            <div className="sm:col-span-2 border border-white/10 rounded-xl p-3 space-y-2">
              ...
              {f.video ? (
                ...
                <div className="flex flex-wrap gap-3 items-center">
                  <label className={`btn-ghost cursor-pointer !py-2 !px-4 ${uploading==="video"?"opacity-60 pointer-events-none":""}`}>
                    {uploading==="video"?<Loader2 className="w-4 h-4 animate-spin"/>:<Film className="w-4 h-4"/>} {uploading==="video"?`Uploading ${uploadPct}%`:"Upload video"}
                    <input type="file" accept="video/*" className="hidden" onChange={e=>handleFileUpload(e,"video")} disabled={uploading!=null}/>
                  </label>
                  <span className="text-xs text-white/40">or paste URL:</span>
          ...
          <div className="sm:col-span-2 border-t border-white/10 pt-3">
            ...
            <div className="flex flex-wrap gap-2 mt-2 items-center">
              <label className={`btn-ghost cursor-pointer !py-1.5 !px-3 !text-xs ${uploading==="attachment"?"opacity-60 pointer-events-none":""}`}>
                {uploading==="attachment"?<Loader2 className="w-3.5 h-3.5 animate-spin"/>:<Upload className="w-3.5 h-3.5"/>} {uploading==="attachment"?`${uploadPct}%`:"Upload file"}
                <input type="file" accept=".pdf,.docx,.pptx,.xlsx,.zip,.txt" className="hidden" onChange={e=>handleFileUpload(e,"attachment")} disabled={uploading!=null}/>
              </label>

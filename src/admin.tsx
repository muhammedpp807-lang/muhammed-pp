      ...
      <aside className="glass p-3 h-fit lg:sticky lg:top-28">
        ...
        <nav className="mt-2 space-y-0.5">
          {TABS.filter(t => currentUser.role==="superadmin" || !["users"].includes(t.k)).map(t=>(
            <button key={t.k} onClick={()=>setTab(t.k)} className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm ${tab===t.k?"bg-cyan-400/20 text-cyan-200":"hover:bg-white/5 text-white/75"}`}><span>{t.i}</span>{t.l}</button>
          ))}
        </nav>
      </aside>
      <div>
    ...
    <motion.div initial={{opacity:0}} animate={{opacity:1}} className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm grid place-items-center p-4" onClick={onClose}>
      <motion.div initial={{scale:.9}} animate={{scale:1}} className="glass w-full max-w-2xl p-6 max-h-[90vh] overflow-auto" onClick={e=>e.stopPropagation()}>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <input className="input sm:col-span-2" placeholder="Image URL" value={f.image} onChange={e=>setF({...f,image:e.target.value})}/>
          ...
          {type==="project" && <><input className="input" placeholder="Live demo URL" value={f.demo} onChange={e=>setF({...f,demo:e.target.value})}/><input className="input" placeholder="Source URL" value={f.source} onChange={e=>setF({...f,source:e.target.value})}/></>}
          <label className="flex items-center gap-2 sm:col-span-2 text-sm"><input type="checkbox" checked={f.featured} onChange={e=>setF({...f,featured:e.target.checked})}/>Featured</label>
        </div>
        <div className="flex justify-end gap-2 mt-4">
          <button onClick={onClose} className="btn-ghost" data-cursor="hover">Cancel</button>
          <button onClick={()=>onSave({ ...f, tags: f.tags ? f.tags.split(",").map((t:string)=>t.trim()).filter(Boolean) : [] })} className="btn-neon" data-cursor="hover"><Check className="w-4 h-4"/>Save</button>
        </div>
      </motion.div>
    </motion.div>
  );
...
function CMSManager() {
  ...
  return (<>
    ...
    <div className="glass p-6 space-y-4">
      ...
      <div className="flex justify-end"><button onClick={()=>{ setAbout(a); alert("Saved!"); }} className="btn-neon" data-cursor="hover"><Check className="w-4 h-4"/>Save changes</button></div>
    </div>
  </>);
}

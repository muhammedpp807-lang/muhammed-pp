...
export function AuthPage({ mode, setMode, setPage }:{mode:"login"|"register"; setMode:(m:"login"|"register")=>void; setPage:(p:string)=>void}) {
  ...
  const { signIn, signUp, resetPassword, fbLoading } = useFirebase();
    ...
    try {
      if (mode === "login") {
        ...
        try {
          ...
          const r = login(f.email, f.password);
          if (r) setErr("Invalid email or password. Ensure Firebase is enabled or use demo accounts.");
          else setPage("home");
        }
      } else {
        try {
          await signUp(f.email, f.password, f.name);
          const r = register(f.name, f.email, f.password);
          if (r) setErr(r); else setPage("home");
        } catch (fbErr: any) {
          if (fbErr.code === "auth/email-already-in-use") {
            setErr("Email already registered in Firebase.");
          } else {
            const r = register(f.name, f.email, f.password);
            if (r) setErr(r); else setPage("home");
          }
    ...
    <section className="min-h-screen grid place-items-center px-4 py-12">
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="max-w-md w-full glass p-6 md:p-8">
        ...
        <form onSubmit={submit} className="space-y-3 mt-5">
          ...
          {resetSent && <div className="text-sm text-emerald-300">Password reset link sent! Check your inbox.</div>}
          <button disabled={loading} className="btn-neon w-full justify-center disabled:opacity-50" data-cursor="hover">
            {loading ? <><span className="w-4 h-4 rounded-full border-2 border-cyan-400 border-t-transparent animate-spin"/>Processing...</> : mode==="login"?<><LogIn className="w-4 h-4"/>Sign In</>:<><UserPlus className="w-4 h-4"/>Create Account</>}
          </button>
        </form>
        ...
        {mode==="login" && <div className="text-center mt-2"><button onClick={handleReset} className="text-xs text-white/50 hover:text-cyan-300 underline">Forgot password?</button></div>}
        <div className="divider my-4"/>
        ...
        <div className="flex items-center justify-center gap-2 mt-3 text-xs text-white/40">
          <span className={`w-2 h-2 rounded-full ${!fbLoading?"bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,.6)]":"bg-amber-400 animate-pulse"}`}/>
          Firebase {!fbLoading ? "Connected" : "Initializing..."}
        </div>
        {mode==="login" && <div className="mt-3 p-3 text-xs glass text-white/70">

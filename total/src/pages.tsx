...
import {
  ...
  Rocket, Shield, Code, Eye, Download, GraduationCap, Target, Sparkles as SparkleIcon, BookOpen
} from "lucide-react";
...
export function HomePage({ setPage }:{setPage:(p:string)=>void}) {
  ...
  return (<>
    ...
    {currentUser && (currentUser.role==="admin"||currentUser.role==="superadmin") && (
      <div className="mx-auto max-w-7xl px-4 md:px-6 pt-16 pb-0">
        <motion.div initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} className="glass border-l-4 border-cyan-400 p-4 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl grid place-items-center bg-gradient-to-br from-cyan-500/40 to-purple-500/40"><SparkleIcon className="w-5 h-5 text-cyan-300"/></div>
            <div><div className="font-display text-sm">Welcome back, {currentUser.name.split(" ")[0]}</div><div className="text-xs text-white/60">You have admin access — edit anything on this site.</div></div>
          </div>
          <button onClick={()=>setPage("admin")} className="btn-neon" data-cursor="hover">Open Admin Panel</button>
        </motion.div>
      </div>
    )}
    {/* HERO */}
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
        {about.skills.map((g,i)=>(
          <motion.div key={i} initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} transition={{delay:i*.1}} viewport={{once:true}} className="glass glass-hover p-5">
            <div className="w-12 h-12 rounded-xl grid place-items-center mb-4 bg-gradient-to-br from-cyan-500/30 to-cyan-700/10 text-cyan-200"><Code className="w-5 h-5"/></div>
            <div className="font-display text-lg mb-3">{g.category}</div>
            <ul className="space-y-2">
              {g.items.map((sk)=> <li key={sk} className="flex items-center gap-2 text-sm text-white/75"><span className="w-1.5 h-1.5 rounded-full bg-cyan-400"/>{sk}</li>)}
            </ul>
          </motion.div>
        ))}
      </div>
